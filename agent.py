...
class Agent:
    ...
    def learn(self):
        ...
        if self.useGPU:
            ...
            max_next_q = torch.cuda.FloatTensor(
                next_q_values[[range(0, self.batch_size)], [actions]]
            )
            current_q = torch.cuda.FloatTensor(
                self.dqn(states)[[range(0, self.batch_size)], [actions]]
            )
            expected_q = rewards.to(self.device) + (self.gamma * max_next_q).to(
                self.device
            )
        else:
            ...
            while True:
                ...
                if done:
                    ...
                    writer.add_scalars(
                        "General Look",
                        {
                            "epsilon_value": self.eps_threshold,
                            "score": score,
                            "reward": reward,
                        },
                        self.episode,
